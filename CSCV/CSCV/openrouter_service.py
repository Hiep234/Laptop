...
from config import OPENROUTER_MODEL, OPENROUTER_API_KEY, logger
...
# ================================================================
def call_openrouter(messages, temperature=0.35, max_tokens=2048):
    """Gửi request đến OpenRouter API bằng requests library."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error("OpenRouter error: status_code=%d, body=%s", response.status_code, response.text)
            if response.status_code == 401:
                return "⚠️ Lỗi API Key OpenRouter. Vui lòng kiểm tra lại OPENROUTER_API_KEY trong file .env."
            elif response.status_code == 429:
                return "⚠️ Hết quota hoặc bị giới hạn lượt gọi OpenRouter API. Vui lòng thử lại sau ít phút."
            elif response.status_code == 404:
                return f"⚠️ Model '{OPENROUTER_MODEL}' không tồn tại hoặc không khả dụng trên OpenRouter."
            return f"⚠️ Lỗi OpenRouter API (Mã lỗi {response.status_code}): {response.text}"
    except requests.exceptions.Timeout:
        logger.error("Timeout connecting to OpenRouter API")
        return "⚠️ Yêu cầu tới OpenRouter bị quá thời gian (timeout). Vui lòng thử lại sau."
    except Exception as e:
        logger.error("Request to OpenRouter failed: %s", e)
        return f"⚠️ Lỗi kết nối OpenRouter: {e}"

def openrouter_chat_with_memory(user_text: str, context_text: str, memory):
    """Gọi OpenRouter với reviews, discounts, preferences, và conversation history."""
    ...
    # Inject preference summary nếu có (giúp AI nhớ user thích gì)
    pref_summary = memory.get_preference_summary()
    if pref_summary:
        messages.append({"role": "user", "content": f"[CONTEXT TỪ CÁC LƯỢT TRƯỚC]\n{pref_summary}"})
        messages.append({"role": "assistant", "content": "Đã ghi nhận thông tin. Tôi sẽ dùng context này để tư vấn phù hợp hơn."})

    # Lấy lịch sử hội thoại (lấy tối đa 8 messages gần nhất)
    history = memory.get_messages_for_llm()
    for msg in history[-8:]:
        role = "user" if msg["role"] == "user" else "assistant"
        messages.append({"role": role, "content": msg["content"]})

    # Tin nhắn hiện tại + product context (RAG) + discounts
    ...
    # Đưa tin nhắn hiện tại vào list messages
    messages.append({"role": "user", "content": current_msg})

    return call_openrouter(messages, temperature=0.35, max_tokens=2048)
