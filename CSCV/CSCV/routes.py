...
from config import app, limiter, _sessions, TOP_K, DB_URI, OPENROUTER_MODEL, logger
from config import get_or_create_session
from nlp_utils import autocorrect_vi
from search import smart_search
from recommender import get_recommendations
from openrouter_service import build_context, openrouter_chat_with_memory
...
def api_recommendations():
    ...
    ans = openrouter_chat_with_memory(
...
def _process_message(msg: str, memory) -> dict:
    ...
    if intent == "followup" and memory.last_option_ids:
        ...
        ans = openrouter_chat_with_memory(corrected_msg + segment_hint, ctx, memory)
        memory.add_assistant(ans)
    ...
    if intent == "general":
        ...
        ans = openrouter_chat_with_memory(corrected_msg + segment_hint, old_ctx, memory)
        memory.add_assistant(ans)
    ...
    # ---- Build enriched message for OpenRouter ----
    ...
    ans = openrouter_chat_with_memory(enriched_msg, ctx, memory)
    memory.add_assistant(ans)
