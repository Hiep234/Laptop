...
const ChatAdmin = () => {
    ...
    const activeUserRef = useRef(null); // track active user để tránh stale closure
    ...
    const loadingForUser = useRef(null); // track user nào đang được load
    ...
    useEffect(() => {
        // Chỉ cập nhật nếu dữ liệu trả về đúng với user đang active
        if (loadingForUser.current !== activeUserRef.current) return;

        if (messagesFromStore.length > 0) {
            if (currentPage === 0) {
                // Trang đầu tiên: thay thế hoàn toàn (không merge)
                setMessages(messagesFromStore);
            } else {
                // Tải thêm trang cũ hơn: prepend, deduplicate
                setMessages((prevMessages) => {
                    const mergedMessages = [...messagesFromStore, ...prevMessages];
            ...
            // Không có tin nhắn nào cho user này
            setMessages([]);
        }
    }, [messagesFromStore]);
    ...
    useEffect(() => {
        if (activeUser) {
            loadingForUser.current = activeUser;
    ...
    useEffect(() => {
        if (users.length > 0 && !activeUser) {
            activeUserRef.current = users[0].id;
            loadingForUser.current = users[0].id;
            setActiveUser(users[0].id);
        }
    }, [users, activeUser]);

    const selectUser = useCallback((userId) => {
        if (activeUserRef.current === userId) return; // tránh reload khi click cùng user

        // Xóa sạch messages ngay lập tức để tránh hiển thị tin nhắn của user cũ
        setMessages([]);
        setCurrentPage(0);
        setHasMore(true);
        userScrolling.current = false;

        // Cập nhật ref trước khi setState để các effect biết user mới
        activeUserRef.current = userId;
        loadingForUser.current = userId;
